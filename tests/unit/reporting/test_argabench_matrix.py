...
import arga_twins_benchmark.reporting.argabench_matrix as matrix
...
MODEL_MATRIX_PATH = ROOT / "benchmark" / "argabench_40" / "model_matrix.json"
CALIBRATION_PATH = ROOT / "benchmark" / "argabench_40" / "historical_fable_5_high_fairness_calibration.json"
...
def test_classifier_uses_selected_matrix_tasks(tmp_path: Path) -> None:
    ...
    report = classify_argabench_matrix(
        matrix_dir,
...
def _classify(matrix_dir: Path) -> dict[str, Any]:
    return classify_argabench_matrix(
        matrix_dir,
        suite_path=SUITE_PATH,
        model_matrix_path=MODEL_MATRIX_PATH,
        historical_calibration_path=CALIBRATION_PATH,
...
def test_offline_classifier_separates_execution_terminal_and_fail_closed_evidence(
    ...
    assert report["protocol"] == ARGABENCH_MATRIX_CLASSIFICATION_PROTOCOL
...
def test_legacy_attempt_number_is_accepted_only_with_unambiguous_zero_invocation_history(
    ...
    _write_json(
        ...
        {
            "protocol": "argabench-retry-archive/1",
            "archive_number": 1,
...
def test_report_writer_refuses_to_mutate_preserved_matrix(tmp_path: Path) -> None:
    ...
    with pytest.raises(ArgaBenchMatrixClassificationError, match="inside the preserved matrix run"):
        write_argabench_matrix_report(
            report,
    ...
    write_argabench_matrix_report(report, output, source_matrix_dir=matrix_dir)
