...
import arga_twins_benchmark.reporting.argabench_crm_legacy as legacy
...
TASKS_PATH = ROOT / "benchmark" / "argabench_40" / "TASKS.md"
CALIBRATION_PATH = ROOT / "benchmark" / "argabench_40" / "historical_fable_5_high_fairness_calibration.json"
FIXTURE_ARCHIVE = ROOT / "tests" / "fixtures" / "argabench_crm_legacy" / "historical-fable-5-high-crm.tar.gz"
...
def _grade(task_dir: Path) -> dict[str, Any]:
    return grade_argabench_crm_legacy(
        task_dir,
...
def test_legacy_grader_matches_all_eight_historical_human_verdicts(
    ...
    assert all(report["protocol"] == ARGABENCH_CRM_LEGACY_PROTOCOL for report in reports.values())
