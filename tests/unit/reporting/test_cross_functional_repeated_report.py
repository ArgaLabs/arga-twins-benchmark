...
from arga_twins_benchmark.reporting.cross_functional_repeated_report import (
...
def test_rejects_a_reused_candidate_run() -> None:
    ...
    with pytest.raises(CrossFunctionalRepeatedReportError, match="reused a candidate run"):
...
def test_rejects_an_excluded_trial() -> None:
    ...
    with pytest.raises(CrossFunctionalRepeatedReportError, match="excluded trial"):
        build_cross_functional_repeated_report(reports, bootstrap_resamples=10)
...
def test_writes_manifest_with_hashed_repeat_sources(tmp_path: Path) -> None:
    ...
    repeated = build_cross_functional_repeated_report(reports, bootstrap_resamples=10)

    outputs = write_cross_functional_repeated_report(
        repeated,
        tmp_path / "published",
