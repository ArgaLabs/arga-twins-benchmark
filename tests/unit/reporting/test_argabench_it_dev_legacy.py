...
import arga_twins_benchmark.reporting.argabench_it_dev_legacy as legacy
from arga_twins_benchmark.reporting.argabench_it_dev_legacy import (
    ARGABENCH_IT_DEV_LEGACY_PROTOCOL,
...
def _historical_run() -> Path:
    configured = os.environ.get("ARGA_ARGABENCH_FABLE_HIGH_RUN")
    ...
    if not all(path.exists() for path in required):
        pytest.skip(
            ...
            "ARGA_ARGABENCH_FABLE_HIGH_RUN to run the calibration test"
        )
    return run_dir
...
def test_matches_all_16_historical_human_verdicts_without_model_oracle_input() -> None:
    ...
    assert all(result["protocol"] == ARGABENCH_IT_DEV_LEGACY_PROTOCOL for result in results.values())
    assert all(
        assertion["evidence"]
