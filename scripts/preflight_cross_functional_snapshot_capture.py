#!/usr/bin/env python3
"""Validate Cross-Functional 40 trusted snapshots without invoking a model."""
...
FAIR_GRADER_PATH = (
    ROOT / "src" / "arga_twins_benchmark" / "reporting" / "cross_functional_fair.py"
)
    ...
    async with semaphore:
        ...
        try:
            ...
            write_private_json(
                ...
                {
                    "protocol": "arga-bench-cross-functional-snapshot-preflight-diff/1",
        ...
        result = {
            "protocol": "arga-bench-cross-functional-snapshot-preflight-task/1",
            "task_id": task_id,
            "domain": task["domain"],
            "scenario_id": scenario_id,
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    if not isinstance(tasks_raw, list):
        raise ValueError("Cross-Functional preflight requires exactly 40 tasks")
    ...
    if len(tasks) != 40:
        raise ValueError("every Cross-Functional task must be a JSON object")
    ...
    write_private_json(
        ...
        {
            "protocol": "arga-bench-cross-functional-snapshot-preflight/1",
    ...
    summary: dict[str, Any] = {
        "protocol": "arga-bench-cross-functional-snapshot-preflight-summary/1",
        "suite_id": typed_suite.get("suite_id"),
