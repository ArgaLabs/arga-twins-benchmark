#!/usr/bin/env python3
"""Run every ArgaBench model profile with a global trial cap."""
...
MATRIX_PATH = ROOT / "benchmark" / "argabench_40" / "model_matrix.json"
...
def load_profiles() -> list[dict[str, Any]]:
    ...
    if len(profiles) != 37 or len({str(item.get("id")) for item in profiles}) != 37:
        raise ValueError("model matrix must contain exactly 37 unique profiles")
    return profiles
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    if resume_existing:
        ...
        if config.get("suite_id") != "argabench-40-v1":
