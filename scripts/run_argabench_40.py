#!/usr/bin/env python3
"""Run the checked-in ArgaBench suite against saved Arga Scenarios."""
        ...
        if task_dir.exists():
            ...
            if not cleanup_ok:
                ...
                write_private_json(
                    ...
                    {
                        "protocol": "argabench-resume-blocked/1",
                        "task_id": task_id,
                        "profile_id": profile_id,
...
def validate_resume_run_config(
    ...
    *,
    ...
    if config.get("protocol") != "argabench-run/2":
        raise ValueError("cannot resume: run-config protocol does not match ArgaBench")
...
def _write_resume_history(
    ...
    *,
    ...
    history = read_json_object(history_path) or {
        "protocol": "argabench-resume-history/1",
        "entries": [],
    }
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    if existing_scenarios is not None and existing_scenarios != scenario_payload:
        ...
        write_private_json(
            ...
            {
                "protocol": "argabench-scenario-resolution/1",
                "suite_tag": SUITE_TAG,
                "resolved_at": utc_now(),
    ...
    summary = {
        "protocol": "argabench-run-summary/2",
        "suite_id": suite["suite_id"],
        "profile": profile,
        "concurrency": args.concurrency,
