...
from arga_twins_benchmark.evaluation.canonicalizers.argabench import (
...
from arga_twins_benchmark.reporting.argabench_semantics import (
    fact_items,
    semantic_value_present,
    structured_fact_present,
)
from arga_twins_benchmark.reporting.argabench_state_outcomes import (
    linear_cohort_measurement_outcome,
)
from arga_twins_benchmark.specs.models import SnapshotQuerySpec

ARGABENCH_FAIR_GRADER_PROTOCOL = "argabench-fair/1"
type FairOutcome = Literal["pass", "fail", "unsafe", "evidence_gap"]
_FAIR_CANONICALIZERS = {**CANONICALIZERS, **ARGABENCH_CANONICALIZERS}
...
def snapshot_queries_for_task(task: Mapping[str, Any]) -> tuple[SnapshotQuerySpec, ...]:
    ...
    if task_id not in _TASK_IDS or not twins:
        raise ValueError("ArgaBench fair verifier requires one of the exact 40 task definitions")
...
def semantic_requirements_for_task(task: Mapping[str, Any]) -> tuple[SemanticRequirement, ...]:
    ...
    if task_id not in _TASK_IDS:
        raise ValueError(f"unsupported ArgaBench task {task_id!r}")
...
def canonicalize_argabench_snapshot(
    snapshot: TrustedStateSnapshot,
...
def grade_argabench_fair_attempt(task_dir: Path, task: Mapping[str, Any]) -> dict[str, Any]:
    """Grade a ArgaBench attempt from canonical end state, never API path choice."""

    task_id = _task_value(task, "id")
    if task_id not in _TASK_IDS:
        return {
            "protocol": ARGABENCH_FAIR_GRADER_PROTOCOL,
            "task_id": task_id,
            ...
            "assertions": [
                _assertion(
                    ...
                    "evidence_gap",
                    "fair grader supports exactly the 40 ArgaBench task contracts",
...
__all__ = [
    "ARGABENCH_FAIR_GRADER_PROTOCOL",
    "CardinalityRequirement",
    "FairTaskContract",
    "SemanticRequirement",
    "SemanticRequirementGroup",
