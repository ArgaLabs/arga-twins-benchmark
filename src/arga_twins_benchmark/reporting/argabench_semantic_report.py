...
from arga_twins_benchmark.reporting.argabench_crm_legacy import (
...
from arga_twins_benchmark.reporting.argabench_matrix import (
    ARGABENCH_MATRIX_CLASSIFICATION_PROTOCOL,
    classify_argabench_matrix,
)
from arga_twins_benchmark.reporting.argabench_mkt_ecom_legacy import (
    grade_mkt_ecom_legacy_attempt,
)

ARGABENCH_SEMANTIC_REPORT_PROTOCOL = "argabench-semantic-report/1"
ARGABENCH_RESULTS_PROTOCOL = "argabench-results/2"
ARGABENCH_PUBLICATION_MANIFEST_PROTOCOL = "argabench-publication-manifest/1"
...
class ArgaBenchSemanticReportError(ValueError):
...
def _select_task_grade(
    *,
    ...
    state_grade = grade_argabench_fair_attempt(task_dir, task)
...
def _crm_grader(*, suite_path: Path, tasks_path: Path) -> DomainGradeCallable:
    def grade(task_dir: Path, task: Mapping[str, Any]) -> DomainGrade:
        task_grade = grade_argabench_crm_legacy(
            task_dir,
    ...
    for grader in graders:
        for prefix in grader.prefixes:
            if prefix in registry:
                raise ArgaBenchSemanticReportError(f"duplicate domain grader for {prefix}")
...
def _load_object(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise ArgaBenchSemanticReportError(f"cannot read {label} {path}: {error}") from error
    if not isinstance(payload, dict):
        raise ArgaBenchSemanticReportError(f"{label} must be a JSON object")
...
def _sha256_path(path: Path) -> str:
    try:
        ...
        raise ArgaBenchSemanticReportError(f"cannot hash {path}: {error}") from error
...
def _suite_tasks(suite: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if suite.get("suite_id") != "argabench-40-v1" or not isinstance(tasks, list):
        raise ArgaBenchSemanticReportError("suite is not ArgaBench v1")
    ...
    if (
        ...
        raise ArgaBenchSemanticReportError("suite must contain exactly 40 unique task objects")
    return typed
...
def _tasks_md_prompts(tasks_path: Path) -> dict[str, str]:
    try:
        ...
        raise ArgaBenchSemanticReportError(f"cannot read TASKS.md: {error}") from error
    ...
    def flush() -> None:
        ...
        if task_id is not None:
            ...
            if not prompt:
                raise ArgaBenchSemanticReportError(f"TASKS.md has no prompt for {task_id}")
            if task_id in prompts:
                raise ArgaBenchSemanticReportError(f"TASKS.md has a duplicate section for {task_id}")
...
def _validate_prompt_contract(tasks: Sequence[Mapping[str, Any]], tasks_path: Path) -> None:
    ...
    if set(prompts) != task_ids:
        raise ArgaBenchSemanticReportError("TASKS.md does not cover the exact 40-task suite")
    mismatches = [
        task_id
    ...
    if mismatches:
        raise ArgaBenchSemanticReportError(f"suite prompts differ from TASKS.md for: {', '.join(mismatches)}")


def _profile_map(model_matrix: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    raw_profiles = model_matrix.get("profiles")
    if not isinstance(raw_profiles, list):
        raise ArgaBenchSemanticReportError("model matrix has no profiles")
    profiles: dict[str, dict[str, Any]] = {}
    for raw_profile in cast(list[object], raw_profiles):
        if not isinstance(raw_profile, dict):
            raise ArgaBenchSemanticReportError("model matrix profile is not an object")
        profile = cast(dict[str, Any], raw_profile)
        profile_id = profile.get("id")
        if not isinstance(profile_id, str) or not profile_id or profile_id in profiles:
            raise ArgaBenchSemanticReportError("model matrix has an invalid profile id")
        if any(field not in profile for field in _PROFILE_FIELDS):
            raise ArgaBenchSemanticReportError(f"model matrix profile {profile_id} is incomplete")
        profiles[profile_id] = profile
    if len(profiles) != _EXPECTED_PROFILE_COUNT:
        raise ArgaBenchSemanticReportError(
...
def build_argabench_semantic_report(
    matrix_dir: Path,
    *,
    suite_path: Path,
    tasks_path: Path,
    ...
    execution_classifier: ExecutionClassifier = classify_argabench_matrix,
) -> dict[str, Any]:
    ...
    suite = _load_object(suite_path, label="ArgaBench suite")
    ...
    if classification.get("protocol") != ARGABENCH_MATRIX_CLASSIFICATION_PROTOCOL:
        raise ArgaBenchSemanticReportError("execution classifier returned an unsupported protocol")
    raw_classified_attempts = classification.get("attempts")
    if not isinstance(raw_classified_attempts, list):
        raise ArgaBenchSemanticReportError("execution classifier must return scheduled slots")
    ...
    if raw_task_ids is None:
        ...
        raise ArgaBenchSemanticReportError("execution classifier returned invalid task ids")
    ...
    for raw_attempt in classified_attempts:
        if not isinstance(raw_attempt, dict):
            raise ArgaBenchSemanticReportError("execution classifier returned a non-object slot")
        ...
        if task_id not in tasks_by_id or profile_id not in profiles:
            raise ArgaBenchSemanticReportError("execution classifier returned an unknown task/profile")
    ...
    if len(identities) != expected_slots:
        raise ArgaBenchSemanticReportError("semantic schedule has duplicate profile/task slots")
    ...
    return {
        ...
        "policy": {
            "execution_classifier_protocol": ARGABENCH_MATRIX_CLASSIFICATION_PROTOCOL,
...
def select_argabench_semantic_report(report: Mapping[str, Any], task_ids: Sequence[str]) -> dict[str, Any]:
    ...
    if not selected_task_ids or len(selected_task_ids) != len(set(selected_task_ids)):
        raise ArgaBenchSemanticReportError("selected task ids must be non-empty and unique")
    ...
    if not isinstance(raw_task_ids, list) or not all(isinstance(task_id, str) for task_id in raw_task_ids):
        raise ArgaBenchSemanticReportError("report is missing selected task ids")
    available_task_ids = cast(list[str], raw_task_ids)
    if not set(selected_task_ids).issubset(available_task_ids):
        raise ArgaBenchSemanticReportError("selected task ids are not present in the source report")
    ...
    if not isinstance(raw_attempts, list) or not isinstance(raw_profiles, Mapping):
        raise ArgaBenchSemanticReportError("report is missing attempts or profiles")
    attempts = [
    ...
    for profile_id, raw_profile in cast(Mapping[str, object], raw_profiles).items():
        if not isinstance(profile_id, str) or not isinstance(raw_profile, Mapping):
            raise ArgaBenchSemanticReportError("report contains a malformed profile")
...
def _site_result(
    ...
    *,
    ...
    if profile_report.get("scoring_ready") is not True:
        raise ArgaBenchSemanticReportError(f"profile {profile_id} is not scoring-ready")
    ...
    if len(by_task) != task_count:
        raise ArgaBenchSemanticReportError(f"profile {profile_id} does not have exactly {task_count} results")
    ...
    return {
        "protocol": ARGABENCH_RESULTS_PROTOCOL,
...
def write_argabench_semantic_report(
    report: dict[str, Any],
    output_dir: Path,
    *,
    ...
    if (
        report.get("protocol") != ARGABENCH_SEMANTIC_REPORT_PROTOCOL
        or not isinstance(report_source, str)
        or Path(report_source).resolve() != source
    ):
        raise ArgaBenchSemanticReportError("report identity does not match the preserved source matrix")
    ...
    if re.fullmatch(r"\d{4}-\d{2}-\d{2}", publication_date) is None:
        raise ArgaBenchSemanticReportError("published_at must use YYYY-MM-DD")
    suite = _load_object(suite_path.resolve(), label="ArgaBench suite")
    ...
    if report.get("suite_id") != suite.get("suite_id"):
        raise ArgaBenchSemanticReportError("report suite identity does not match suite.json")
