...
ARGABENCH_IT_DEV_LEGACY_PROTOCOL = "argabench-it-dev-legacy-grade/1"
...
def grade_it_dev_legacy_task(*, task: Mapping[str, Any], task_dir: Path) -> dict[str, Any]:
    ...
    if not isinstance(task_id, str) or task_id not in _RULES:
        return {
            "protocol": ARGABENCH_IT_DEV_LEGACY_PROTOCOL,
            "task_id": task_id,
...
def load_it_dev_legacy_tasks(suite_path: Path) -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        raise ValueError(f"cannot read trusted ArgaBench suite: {error}") from error
    if not isinstance(payload, dict) or payload.get("suite_id") != "argabench-40-v1":
        raise ValueError("trusted suite identity is not argabench-40-v1")
