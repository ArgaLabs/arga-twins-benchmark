...
class ArgaBenchRepeatedReportError(ValueError):
...
def _percentile(sorted_values: Sequence[float], probability: float) -> float:
    if not sorted_values:
        raise ArgaBenchRepeatedReportError("cannot calculate a percentile without samples")
...
def _bootstrap_task_cluster_ci(
    ...
    *,
    ...
    if task_count < 1:
        raise ArgaBenchRepeatedReportError("bootstrap requires at least one task cluster")
    if resamples < 1:
        raise ArgaBenchRepeatedReportError("bootstrap resamples must be positive")
...
def _usage(attempts: Sequence[Mapping[str, Any]]) -> dict[str, int | float]:
    ...
    for field in _USAGE_FIELDS:
        ...
        if not all(isinstance(value, (int, float)) and not isinstance(value, bool) for value in values):
            raise ArgaBenchRepeatedReportError(f"every repeated trial requires metric {field}")
...
def _validate_report(
    ...
    *,
    ...
    if report.get("protocol") != ARGABENCH_SEMANTIC_REPORT_PROTOCOL:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} has an unsupported report protocol")
    if report.get("suite_id") != "argabench-40-v1":
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} changed suite identity")
    if report.get("matrix_scoring_ready") is not True:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} is not matrix-scoring-ready")
    if report.get("scoring_ready_profile_count") != _EXPECTED_PROFILE_COUNT:
        raise ArgaBenchRepeatedReportError(
            f"repeat {repeat} must contain {_EXPECTED_PROFILE_COUNT} scoring-ready profiles"
        )
    source_hashes = report.get("source_sha256")
    if not isinstance(source_hashes, Mapping):
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} is missing source hashes")
    ...
    if expected_hashes is not None and dict(typed_source_hashes) != dict(expected_hashes):
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} changed the suite, task, or profile inputs")
    grader_provenance = report.get("grader_provenance")
    if not isinstance(grader_provenance, Mapping):
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} is missing executable grader provenance")
    ...
    if not isinstance(bundle_sha256, str) or len(bundle_sha256) != 64:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} has invalid grader provenance")
    if expected_grader_provenance is not None and dict(typed_grader_provenance) != dict(expected_grader_provenance):
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} changed the executable grader revision")
    ...
    if len(profiles) != _EXPECTED_PROFILE_COUNT:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains a malformed profile")
    if any(profile.get("scoring_ready") is not True for profile in profiles.values()):
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains a non-scoring profile")

    raw_attempts = report.get("attempts")
    if not isinstance(raw_attempts, list):
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} must contain attempts")
    ...
    if len(attempts) != len(typed_raw_attempts) or not attempts:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains a malformed attempt")
    ...
    if raw_task_ids is None:
        ...
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} has invalid selected task ids")
    if (
        len(task_ids) != len(set(task_ids))
        or set(task_ids) != set(observed_task_ids)
        or (report.get("task_count") is not None and report.get("task_count") != len(task_ids))
    ):
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} changed its selected task set")
    if expected_task_ids is not None and tuple(expected_task_ids) != task_ids:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} changed the selected task set")
    expected_attempts = _EXPECTED_PROFILE_COUNT * len(task_ids)
    if len(attempts) != expected_attempts:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} must contain exactly {expected_attempts} attempts")
    identities = {(item.get("profile_id"), item.get("task_id")) for item in attempts}
    if len(identities) != expected_attempts:
        raise ArgaBenchRepeatedReportError(f"repeat {repeat} has duplicate profile/task slots")
    for item in attempts:
        if item.get("profile_id") not in profiles:
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains an unknown profile")
        if item.get("validity") != "valid" or item.get("score_eligible") is not True:
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains an excluded trial")
        if item.get("semantic_outcome") not in _SEMANTIC_OUTCOMES:
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains an invalid verdict")
        if item.get("cleanup_succeeded") is not True:
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains an unproven teardown")
        metrics = item.get("metrics")
        if not isinstance(metrics, Mapping) or item.get("metric_gaps"):
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} contains incomplete metrics")
    return profiles, attempts, typed_grader_provenance, task_ids


def build_argabench_repeated_report(
    ...
    *,
    ...
    if tuple(sorted(reports)) != _EXPECTED_REPEATS:
        raise ArgaBenchRepeatedReportError("exactly repeats 1, 2, and 3 are required")
        ...
        if reference_profiles is None:
            ...
            for profile_id, profile in profiles.items():
                if profile_id not in reference_profiles:
                    raise ArgaBenchRepeatedReportError(f"repeat {repeat} added profile {profile_id}")
                for field in ("profile", "publication_profile_id"):
                    if profile.get(field) != reference_profiles[profile_id].get(field):
                        raise ArgaBenchRepeatedReportError(f"repeat {repeat} changed {profile_id}.{field}")
    ...
    for profile_index, profile_id in enumerate(reference_profiles):
        ...
        if (
            ...
            raise ArgaBenchRepeatedReportError(f"profile {profile_id} lacks three trials per task")
        ...
        for task_id in sorted(by_task):
            ...
            if not all(isinstance(run_id, str) and run_id for run_id in run_ids):
                raise ArgaBenchRepeatedReportError(f"{profile_id}/{task_id} is missing a run ID")
            if len(set(run_ids)) != len(_EXPECTED_REPEATS):
                raise ArgaBenchRepeatedReportError(f"{profile_id}/{task_id} reused a candidate run")
            ...
            if not all(isinstance(scenario_id, str) and scenario_id for scenario_id in scenario_ids):
                raise ArgaBenchRepeatedReportError(f"{profile_id}/{task_id} is missing a scenario ID")
            ...
            if len(prompt_facts) != 1:
                raise ArgaBenchRepeatedReportError(f"{profile_id}/{task_id} changed task content")
    ...
    return {
        "protocol": ARGABENCH_REPEATED_REPORT_PROTOCOL,
...
def write_argabench_repeated_report(
    report: Mapping[str, Any],
    output_dir: Path,
    *,
    ...
    if report.get("protocol") != ARGABENCH_REPEATED_REPORT_PROTOCOL:
        raise ArgaBenchRepeatedReportError("unsupported repeated report protocol")
    if report.get("all_trials_scoring_ready") is not True:
        raise ArgaBenchRepeatedReportError("refusing to publish incomplete repeat evidence")
    if tuple(sorted(repeat_semantic_reports)) != _EXPECTED_REPEATS:
        raise ArgaBenchRepeatedReportError("publication requires three semantic report paths")
    publication_date = published_at or date.today().isoformat()
    if not publication_date or len(publication_date) != 10:
        raise ArgaBenchRepeatedReportError("published_at must use YYYY-MM-DD")

    output = output_dir.resolve()
    if output.exists() and (not output.is_dir() or any(output.iterdir())):
        raise ArgaBenchRepeatedReportError("output directory must be absent or empty")
    ...
    for repeat in _EXPECTED_REPEATS:
        ...
        if semantic.get("protocol") != ARGABENCH_SEMANTIC_REPORT_PROTOCOL:
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} semantic report changed")
        if publication.get("protocol") != ARGABENCH_PUBLICATION_MANIFEST_PROTOCOL:
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} publication manifest changed")
        if len(publication.get("profiles", [])) != _EXPECTED_PROFILE_COUNT:
            raise ArgaBenchRepeatedReportError(f"repeat {repeat} publication is incomplete")
        sources.append(
            {
                "repeat": repeat,
    ...
    manifest = {
        "protocol": ARGABENCH_REPEATED_PUBLICATION_MANIFEST_PROTOCOL,
