...
from arga_twins_benchmark.reporting.argabench_compat import (
    CURRENT_ATTEMPT_PROTOCOL,
    SUPPORTED_ATTEMPT_PROTOCOLS,
)
from arga_twins_benchmark.reporting.argabench_tool_ceiling import (
    legacy_gateway_ceiling_rejections,
)

ARGABENCH_MATRIX_CLASSIFICATION_PROTOCOL = "argabench-model-matrix-offline-classification/1"
HISTORICAL_CALIBRATION_PROTOCOL = "argabench-historical-calibration/1"
...
_ATTEMPT_PROTOCOL = CURRENT_ATTEMPT_PROTOCOL
_CONTROL_PROTOCOL = "arga-bench-control/1"
_MATRIX_CONFIG_PROTOCOL = "argabench-model-matrix-run/1"
_PROFILE_CONFIG_PROTOCOL = "argabench-run/2"
...
_RETRY_ARCHIVE_PROTOCOL = "argabench-retry-archive/1"
...
class ArgaBenchMatrixClassificationError(ValueError):
...
def _sha256_path(path: Path) -> str:
    try:
        ...
        raise ArgaBenchMatrixClassificationError(f"cannot hash trusted artifact {path}: {error}") from error
...
def _load_trusted_object(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise ArgaBenchMatrixClassificationError(f"cannot read {label} {path}: {error}") from error
    if not isinstance(payload, dict):
        raise ArgaBenchMatrixClassificationError(f"{label} {path} must contain a JSON object")
...
def _profile_by_id(payload: Mapping[str, Any], *, label: str) -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(raw_profiles, list) or not raw_profiles:
        raise ArgaBenchMatrixClassificationError(f"{label} profiles must be a non-empty array")
    profiles: dict[str, dict[str, Any]] = {}
    for raw_profile in cast(list[object], raw_profiles):
        if not isinstance(raw_profile, dict):
            raise ArgaBenchMatrixClassificationError(f"{label} profiles must contain only objects")
        profile = cast(dict[str, Any], raw_profile)
        profile_id = profile.get("id")
        if not _non_empty_string(profile_id) or profile_id in profiles:
            raise ArgaBenchMatrixClassificationError(f"{label} has an invalid or duplicate profile id")
...
def _suite_tasks(suite: Mapping[str, Any]) -> list[dict[str, Any]]:
    if suite.get("suite_id") != "argabench-40-v1":
        raise ArgaBenchMatrixClassificationError("suite identity is not argabench-40-v1")
    raw_tasks = suite.get("tasks")
    if not isinstance(raw_tasks, list):
        raise ArgaBenchMatrixClassificationError("ArgaBench suite must contain exactly 40 tasks")
    ...
    for raw_task in task_items:
        if not isinstance(raw_task, dict):
            raise ArgaBenchMatrixClassificationError("suite tasks must contain only objects")
        ...
        if (
            ...
            raise ArgaBenchMatrixClassificationError("suite contains an invalid task contract")
...
def _validate_calibration(
    ...
    *,
    ...
    if calibration.get("protocol") != HISTORICAL_CALIBRATION_PROTOCOL:
        raise ArgaBenchMatrixClassificationError("historical calibration protocol is unsupported")
    if calibration.get("calibration_only") is not True:
        raise ArgaBenchMatrixClassificationError("historical verdicts must be marked calibration_only")
    profile = calibration.get("profile")
    if not isinstance(profile, dict):
        raise ArgaBenchMatrixClassificationError("historical calibration profile is missing")
    ...
    if any(typed_profile.get(key) != value for key, value in expected_profile.items()):
        raise ArgaBenchMatrixClassificationError("historical calibration is not the Fable 5 High audit")
    verdicts = calibration.get("verdicts")
    if not isinstance(verdicts, dict):
        raise ArgaBenchMatrixClassificationError("historical calibration verdicts are missing")
    typed_verdicts = cast(dict[str, Any], verdicts)
    task_ids = {cast(str, task["id"]) for task in tasks}
    if set(typed_verdicts) != task_ids:
        raise ArgaBenchMatrixClassificationError("historical calibration does not cover the exact suite")
    passed = 0
    for task_id, raw_verdict in typed_verdicts.items():
        if not isinstance(raw_verdict, dict):
            raise ArgaBenchMatrixClassificationError(f"historical calibration verdict {task_id!r} is malformed")
    ...
    *,
    ...
    for archive_number, archive_dir in archives.items():
        ...
        if archived_issues or archived_attempt is None:
            return False
        if (
            archived_attempt.get("protocol") not in SUPPORTED_ATTEMPT_PROTOCOLS
...
def _matrix_config_issues(
    ...
    *,
    ...
    except ArgaBenchMatrixClassificationError:
...
def _profile_inputs(
    ...
    *,
    ...
    if scenarios is not None:
        if scenarios.get("suite_tag") != "suite:argabench-40-v1":
...
def classify_argabench_matrix(
    ...
    *,
    ...
    """Classify a preserved ArgaBench matrix without grading semantics.
    ...
    suite = _load_trusted_object(suite_path, label="ArgaBench suite")
...
def write_argabench_matrix_report(
    report: dict[str, Any],
    output_path: Path,
    *,
    ...
    if output == source or output.is_relative_to(source):
        raise ArgaBenchMatrixClassificationError(
...
__all__ = [
    ...
    "ArgaBenchMatrixClassificationError",
    "classify_argabench_matrix",
    "write_argabench_matrix_report",
]
