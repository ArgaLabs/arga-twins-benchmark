...
PR #1158 now passes final source CI (5,478 twin / 2,130 root tests), both production workflows and fresh hosted color filtering/sorting. Desktop and 390px failure/retry, cancellation, keyboard and reload checks preserve exact protected state, including synchronized Google file metadata through Docs. Hosted saved/temporary native filter-view lifecycle, read-only preview and atomic rollback pass. Both fresh runs have complete ten-container image matches and confirmed teardown; both serving API/proxy revisions match publication records. Saved/temporary filter-view frontend, sorted-view editing/formula/clipboard projection and exhaustive parity remain open.
