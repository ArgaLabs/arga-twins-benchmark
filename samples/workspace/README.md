...
All five tasks retain earlier passing hosted candidate episodes with confirmed teardown. Each candidate had browser workspaces and native API tools available and could choose either mode. A new WKS-03 revalidation failed after the candidate retained superseded pricing without reading the approved Gmail terms (20 seats/$8,400 instead of 24 seats/$8,640). The failure is preserved and correctly classified as `fail`, with no prohibited mutations. Its final-deployment rollout gate is pending; no automatic retry or hidden regrading was applied.
...
The current evidence includes a clean locked install and a later full run with 1,191 benchmark tests passing (26 skipped), candidate/relay/outcome regressions and 18 current package/readiness checks. Input scans contain no live credentials or image files. Historical invocation results are retained in `evidence/regression-log-summaries.json`; overlapping test counts are not additive. Later source changes must refresh hashes and relevant checks before release. No final ZIP has been created.
