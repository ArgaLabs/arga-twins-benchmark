...
Remaining work includes complete route/control inventories and reference/mobile audits, settings/admin/billing/analytics/AI and collaboration surfaces, GitHub advanced review/Projects/Actions, Gmail inline composer and remaining menus, Calendar access requests/side apps, Docs section variants/suggesting/table positioning, Sheets filter views/charts/protection/undo, Notion workspace/guest/advanced database editing, Linear notifications/uploads/realtime, and Stripe reporting/admin. Physical-mobile coverage remains open. Current deployment image attestation passes for the fresh eight-provider run and serving API/proxy revisions. `readiness.json` keeps all eight frontend gates pending.
...
PR #1155 adds native header/footer/footnote editing, segment-aware save/recovery and tab isolation. Its export review fix retains segments across ten formats, with native DOCX headers/footers and linked footnotes. Local browser checks, 23 export regressions and 379 broader Docs/Drive/Workspace checks pass. CI (5,362 twin and 2,106 root tests), both production deployments and fresh hosted native/20-format-variant export checks pass; protected state is unchanged and teardown is confirmed. Fresh hosted desktop/390px GUI checks now pass with native-state assertions, failure recovery, reload and confirmed teardown. Page-exact PDF rendering and broader Docs fidelity remain open. See `docs-document-segments-2026-09-24.json`.

PR #1156 adds Linear native comment threads, author editing, copy links and resolution/reopening. Scenario graph validation now rejects malformed relationships before any reset, and introspection exposes supported thread fields and arguments. All 164 Linear regressions and bounded local desktop/390px flows pass. Full CI (5,392 twin and 2,106 root tests), both production deployments and fresh hosted native/form checks pass, with protected state preserved and teardown confirmed. Fresh hosted desktop/390px GUI checks now pass with native-state assertions, failure recovery, reload and confirmed teardown. See `linear-comment-threads-2026-09-24.json`.

PR #1158 adds native filter-view validation/preview and effective color filtering/sorting. Its earlier 536 Sheets/shared Workspace regressions and 6d796c6a CI (5,476 twin and 2,106 root tests) pass. Color filtering/sorting now passes bounded desktop/390px native-state, cancel, failure/retry, keyboard and reload checks; 115 focused tests and zero-error Pyright cover the latest spill-gradient review fix. The reviewer scored that finding resolved and the ordinary-extension documentation handoff is complete. CI at a653fdb9 is pending. Real Sheets temporary/saved lifecycle, sorted edit mapping and view-dependent formula display were observed using an owned fixture, then trashed. Saved-view frontend, sorted-view editing and hosted color checks remain open; this PR is not deployed. See `sheets-filter-views-colors-2026-09-24.json`.

PR #1157 merged as `19518bc1` after full CI (5,392 twin and 2,130 root tests). Production Runtime and Workers workflows pass. The fresh eight-provider run has all ten expected containers running with complete image identities: all eight provider digests match their published builds, the application matches its builder record, and the gateway matches its official release manifest. Serving API/proxy revisions are ready, receive 100% traffic and match published manifests. Teardown is confirmed. See `runtime-image-evidence-2026-09-24.json`. This is timestamped evidence for this deployment, not an inventory of unrelated production workloads.

PR #1159 adds native Calendar Other notifications preferences with atomic validation, personal reader subscriptions and immediate revocation. All 117 local Calendar tests pass. Desktop and 390px cancellation, failure/retry/reload and delayed-save checks preserve unrelated native state. Independent review scored the pending-save and copy fixes resolved. CI, hosted verification and actual notification delivery remain pending. See `calendar-notification-preferences-2026-09-25.json`.
