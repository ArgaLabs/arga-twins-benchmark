...
Docs document tab navigation/lifecycle, nesting, duplication, drag reordering, outlines, deep links and tab-specific edits/exports are implemented in twin PR #1100, with 190 unique regressions and local Chrome desktop/390px checks. Named-range IDs now remain unique across tabs and duplicated subtrees, including primary-tab deletion. Hosted validation is pending; see `evidence/docs-document-tabs.json`. These feature checks do not complete the Docs or eight-twin frontend audits.
...
Sheets merge-cell controls and rendering are in PR #1101, with 196 regressions plus Chrome desktop/390px checks of warning cancellation, merge modes, unmerge, hidden/offscreen anchors and frozen rows. Border/frozen-pane integration is verified locally in PR #1099, and PR #1101 now combines merged outer borders with native perimeter reads and independent frozen dividers. Hosted merge/border validation and irregular merged-perimeter provider-reference comparison remain pending.

Twin PR #1101 now combines the pending Gmail bulk-selection, Docs tab and Sheets border/merge changes into one deployment candidate. All 573 combined Docs/Sheets/Gmail/Workspace regressions passed, with clean generated JavaScript and Ruff checks. CI/review and hosted validation of that exact combined source remain pending; see `evidence/workspace-integration.json`. Component PRs #1097, #1099 and #1100 remain available until integration is complete.

The combined Workspace PR also passed 70 focused regressions after rejecting malformed merge types with atomic rollback (six new cases). Linear table structure menus are in twin PR #1102: 73 regressions plus Chrome desktop/390px checks cover row/column editing, alignment, keyboard movement, undo/redo and native comment state. Mobile footer spacing keeps Comment clickable. CI, deployment, hosted checks and the broader Linear audit remain pending; see `evidence/linear-table-controls.json`.
