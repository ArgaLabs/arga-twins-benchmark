...
The 24 September reviewed-contract runs add these results: WKS-02 passes all ten assertions with both attendees unchanged; WKS-01 and WKS-04 expose a GitHub GET-state mutation fixed by PR #1137; WKS-04 also required recognition of the valid "not yet approved" mitigation wording. WKS-03 used one browser observation and APIs, then exposed the ambiguous reviewer-recipient policy. Original verdicts, diagnoses and cleanup are retained in `wks-*-reviewed-contract-candidate-2026-09-24.json`. A fresh WKS-03 episode under the explicit recipient policy passes all 12 assertions, with confirmed teardown; it chose APIs while browser access remained available. See `wks-03-explicit-recipient-candidate-2026-09-24.json`. Both GitHub production workflows have passed. A fresh WKS-01 candidate passes all assertions with teardown confirmed; WKS-04 is running. See `wks-01-github-read-fix-candidate-2026-09-24.json`.
...
- PR #1137 fixes requested-reviewer responses and prevents read-only generic GitHub calls from creating state. All 350 GitHub tests and CI pass; merged as `5703d0be`, both production workflows pass, and a fresh WKS-01 candidate passes. WKS-04 is running. See `github-requested-reviewers.json`.
...
- PR #1141 adds Notion Settings/People/Groups with membership and owner controls, plus deterministic Scenario users. All 172 relevant tests and local Chrome desktop/390px flows pass. Review fixes cover case-insensitive auth and correct Settings navigation for restricted tokens and members without pages. CI, deployment and hosted replay remain pending. See `notion-workspace-groups.json`.
