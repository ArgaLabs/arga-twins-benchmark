...
The 24 September reviewed-contract runs add these results: WKS-02 passes all ten assertions with both attendees unchanged; WKS-01 and WKS-04 expose a GitHub GET-state mutation fixed by PR #1137; WKS-04 also required recognition of the valid "not yet approved" mitigation wording. WKS-03 used one browser observation and APIs, then exposed the ambiguous reviewer-recipient policy. Original verdicts, diagnoses and cleanup are retained in `wks-*-reviewed-contract-candidate-2026-09-24.json`. A fresh WKS-03 episode under the explicit recipient policy passes all 12 assertions, with confirmed teardown; it chose APIs while browser access remained available. See `wks-03-explicit-recipient-candidate-2026-09-24.json`. The fresh GitHub deployment still requires WKS-01/WKS-04 reruns.
