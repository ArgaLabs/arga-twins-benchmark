...
The 24 September reviewed-contract runs add these results: WKS-02 passes all ten assertions with both attendees unchanged; the initial WKS-01 and WKS-04 attempts exposed a GitHub GET-state mutation fixed by PR #1137; WKS-04 also required recognition of the valid "not yet approved" mitigation wording. WKS-03 used one browser observation and APIs, then exposed the ambiguous reviewer-recipient policy. Original verdicts, diagnoses and cleanup are retained in `wks-*-reviewed-contract-candidate-2026-09-24.json`. A fresh WKS-03 episode under the explicit recipient policy passes all 12 assertions, with confirmed teardown; it chose APIs while browser access remained available. See `wks-03-explicit-recipient-candidate-2026-09-24.json`. Both GitHub production workflows have passed. A fresh WKS-01 candidate passes all assertions with teardown confirmed; WKS-04 also passes all assertions with teardown confirmed; see `wks-04-github-read-fix-candidate-2026-09-24.json`. See `wks-01-github-read-fix-candidate-2026-09-24.json`.
...
- PR #1136 serves native Linear project/document/issue/comment/profile paths, including archived resources. All 116 Linear tests pass and local Chrome/native checks pass. CI passed and the PR merged at `47243201`; both production workflows passed after retrying a Docker Hub 502. Hosted duplicate-project links, comment/profile navigation and mobile document save/reload pass; the original premature-reload attempt is preserved separately. Native protected-state checks and teardown pass. See `linear-native-links.json` and `linear-native-links-production-2026-09-24.json`.
- PR #1137 fixes requested-reviewer responses and prevents read-only generic GitHub calls from creating state. All 350 GitHub tests and CI pass; merged as `5703d0be`, both production workflows pass, and a fresh WKS-01 candidate passes. WKS-04 also passes all assertions with teardown confirmed; see `wks-04-github-read-fix-candidate-2026-09-24.json`. See `github-requested-reviewers.json`.
- PR #1138 adds typed nested AND/OR filters to Notion views. All 158 Notion tests pass, including the review follow-up for emptiness-operator precedence; desktop and 390px Chrome editing preserve protected records. All CI passed and the PR merged as `98d5960d`; both production workflows and hosted desktop/390px nested-filter save/reload pass. Protected state and teardown checks pass. An initial harness fingerprint-parser failure is preserved separately. See `notion-nested-filters.json` and `notion-nested-filters-production-2026-09-24.json`.
- PR #1124 adds Gmail compose windows and More options. Local Chrome/native integration checks and 217 Gmail regressions pass on the updated main base; current-head CI passes. Native print-dialog/afterprint inspection is blocked by host UI access; no print job was submitted. Production deployment and hosted checks are pending.

- PR #1140 separates Stripe currency balances and dashboard totals, corrects minor-unit formatting and keeps mobile ledger rows readable. All 78 Stripe tests and Chrome desktop/390px checks pass with protected state unchanged. Current-schema baseline failures for three required nullable response fields are fixed; all 55 exercised official-schema endpoints pass (504 unexercised). Current-head CI, deployment and hosted replay remain pending. See `stripe-currency-balances.json`.

- PR #1141 adds Notion Settings/People/Groups with membership and owner controls, plus deterministic Scenario users. All 173 relevant tests and local Chrome desktop/390px flows pass. Review fixes cover case-insensitive auth and correct Settings navigation for restricted tokens and members without pages. CI, deployment and hosted replay remain pending. See `notion-workspace-groups.json`.

- PR #1142 adds rich Google Docs version preview, naming/filtering, restore and copy, shared across Docs and Drive. All 175 Docs/Workspace regressions and local Chrome desktop/390px checks pass. CI, deployment and hosted replay remain pending. See `docs-version-history.json`.

- PR #1143 integrates #1139, #1140, #1141 and #1142 on current main. The combined revision must pass CI and production validation; the source PRs remain available for their review history.
...
- PR #1139 adds Linear project overview properties, native content/members, documents, milestones, and updates. Local Chrome desktop/390px flows pass. Review fixes preserve disabled-update behavior and expose content/members in Scenario generation; 278 combined Linear and Scenario-generator tests pass. The backend schema integration test now runs in the backend dependency environment. CI passes; the branch needs the latest main before merge, deployment and hosted replay. See `linear-project-overview.json`.
