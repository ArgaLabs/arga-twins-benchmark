...
- PR #1124 adds Gmail compose windows and More options. Local Chrome/native integration checks and 217 Gmail regressions pass on the updated main base; current-head CI is running. Native print-dialog/afterprint inspection is blocked by host UI access; no print job was submitted. Production deployment and hosted checks are pending.
...
- PR #1142 adds rich Google Docs version preview, naming/filtering, restore and copy, shared across Docs and Drive. All 174 Docs/Workspace regressions and local Chrome desktop/390px checks pass. CI, deployment and hosted replay remain pending. See `docs-version-history.json`.
