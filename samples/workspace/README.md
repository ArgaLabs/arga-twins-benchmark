...
- PR #1128 adds Docs table border dragging. It passes 308 Docs/Workspace tests plus two additional title-status regressions and 18 focused resize/save-queue tests. Chrome desktop/390px, merged-edge and pending-edit/reload checks pass. The live provider comparison confirms column deltas; exact row geometry differs. Cancellation handler regressions pass, but native cancellation and physical-touch checks remain open. CI passed and the PR merged as `93f4c45d`; production workflows and hosted resize checks are pending. See `docs-table-resize.json` and `docs-table-resize-provider-reference.json`.
